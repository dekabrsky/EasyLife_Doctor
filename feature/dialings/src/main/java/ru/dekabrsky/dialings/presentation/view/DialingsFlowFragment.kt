...
class DialingsFlowFragment : BasicFlowFragment(), DialingsFlowView {
    ...
    private lateinit var args: DialingsFlowScreenArgs
    ...
    override fun provideNavigator(router: AppRouter): FragmentFlowNavigator =
        object : FragmentFlowNavigator(this, router, containerId) {
            override fun createFragment(screenKey: String?, data: Any?): Fragment? =
                when (screenKey) {
                    Flows.Dialing.SCREEN_DIALINGS_LIST -> DialingsListFragment.newInstance()
    ...
    override fun injectDependencies() {
        Toothpick.openScopes(args.parentScope, scopeName)
            .module { bind(DialingsFlowScreenArgs::class.java).toInstance(args) }
            .installNavigation()
            .inject(this)
    }
    ...
    companion object {
        fun newInstance(args: DialingsFlowScreenArgs) = DialingsFlowFragment().apply {
            this.args = args
        }
    }
}